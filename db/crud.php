...
class crud
{
    ...
    {
        try {
            $sql = "UPDATE `newsite` SET `firstname`=:fname,
            `lastname`=:lname,`dateofbirth`=:dob,`emailaddress`=:email,
            `contactnumber`=:contact,`specialty_id`=:specialty WHERE site_id=:id";
            //prepare the sql statement for execution
            $stmt = $this->db->prepare($sql);
    ...
    {
        try {
            $sql = "SELECT * FROM `newsite` a inner join
    ...
    {
        try {
            $sql = "DELETE FROM `newsite` WHERE site_id=:id";
            $stmt = $this->db->prepare($sql);
            $stmt->bindparam(':id', $id);
            $stmt->execute();
            return true;
